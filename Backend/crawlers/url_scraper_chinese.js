...
const crawlChinese = async({ url }) =>{
    url = "http://ccll.cau.ac.kr/dm/dm_1.php";
    let url_list = [];
    let title_list = [];
    ...
    //    fs.appendFileSync('pup.json', `${data}\n`);
        if(element.attribs.href != undefined){
            if(element.attribs.href.startsWith("javascript:view")){ // 24 29
                ...
                url_list.push(`http://ccll.cau.ac.kr/dm/dm_1.php?idx=&p_idx=${trimmedLink}&s_kind=&s_scroll=all&s_key=&p_page=1&code=b_2&p_listtype=&p_mode=view&mode=&p_pgfile=%2Fdm%2Fdm_1.php`)
