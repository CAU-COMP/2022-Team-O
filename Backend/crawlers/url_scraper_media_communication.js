...
const crawlMediaCommunication = async({ url }) =>{
    url = "https://cmc.cau.ac.kr/05_notice/notice_01a.php";
    let url_list = [];
    let title_list = [];
    ...
    //    fs.appendFileSync('pup.json', `${data}\n`);
        if(element.attribs.href != undefined){
            if(element.attribs.href.startsWith("javascript:view")){ // 24 29
                ...
                url_list.push(`https://cmc.cau.ac.kr/05_notice/notice_01a.php?idx=&p_idx=${trimmedLink}&s_kind=&s_scroll=&s_key=&p_page=1&code=b_6&p_listtype=&p_mode=view&mode=&p_pgfile=%2F05_notice%2Fnotice_01a.php`);
...
//crawlMediaCommunication({
 //   url: "url",
//}); // 테스트용

export default crawlMediaCommunication
