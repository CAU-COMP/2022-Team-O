...
const crawlPhysics = async({ url }) =>{
    url = "http://physics.cau.ac.kr/FM/fm_1.php";
    let url_list = [];
    let title_list = [];
    ...
    //    fs.appendFileSync('pup.json', `${data}\n`);
        if(element.attribs.href != undefined){
            if(element.attribs.href.startsWith("javascript:view")){ // 24 29
                ...
                url_list.push(`http://physics.cau.ac.kr/FM/fm_1.php?idx=&p_idx=${trimmedLink}&s_kind=&s_scroll=&s_key=&p_page=1&code=b_4&p_listtype=&p_mode=view&mode=&p_pgfile=%2FFM%2Ffm_1.php`);
